...
uses
  // ------------Clases Utilizadas-----Classes Used-----
    ClassGlobal,
  // -----------------------------------------------------
  ...
  cxGridCardView, cxGridDBCardView, cxGridCustomLayoutView, cxCheckGroup,
  System.JSON, cxCustomListBox, cxCheckListBox, cxCheckComboBox, cxDBCheckGroup,
  Vcl.DdeMan, cxDBCheckListBox, dxDBToggleSwitch, Vcl.CheckLst, Vcl.NumberBox,
  DateUtils, cxImage, Vcl.ExtDlgs, cxImageComboBox, dxSkinWXI;
...
type
  TClasificador = class(TForm)
    ...
    LMainGroup_Root: TdxLayoutGroup;
    LMain: TdxLayoutControl;
    ...
    cxGridDBTableView12Detalles: TcxGridDBColumn;
    ...
    cxGrid11DBTableView1lang: TcxGridDBColumn;
    ...
    procedure btInsertServicesClick(Sender: TObject);
...
var Clasificador: TClasificador;

implementation

uses DataClasif, ConexionDM, DataClasif_U, tasaactual, DCaja, dataCost, dmRates;
{$R *.dfm}

procedure TClasificador.RefreshTable;

begin
  with dmCost do begin

    tbtype_gasto.Refresh;
    tb_typeMP.Refresh;
    tbtype_gasto.Refresh;
  end;
  DM_Clasif_E.tbGastosFijos.Refresh
...
var SP, SI, i, L: Integer; Impar: Boolean;
begin
  L := Length(Ean);
  if (L = 8) or (L = 13) then begin
    SI := 0;
    SP := 0;
    i := L - 1;
    Impar := Odd(L);
    repeat // Bucle para sumar los valores discriminando posiciones pares e impares
      if Impar then SI := SI + StrToInt(Ean[i])
      else SP := SP + StrToInt(Ean[i]);
      Impar := not Impar;
      i := i - 1;
    until i = 0;
    if Impar then SI := SI * 3
    else SP := SP * 3; // Multiplica por tres según la longitud del código
...
function CalculoDigitoControlEAN(Ean: String): Integer;
var SP, SI, i, L: Integer; Impar: Boolean;
begin
  L := Length(Ean);
  SP := 0;
  ...
  for i := L downto 1 do begin
    if Impar then SP := SP + StrToInt(Ean[i])
    else SI := SI + StrToInt(Ean[i]);
    Impar := not Impar;
  end;
  Result := (10 - ((3 * SP + SI) mod 10)) mod 10;
end;
...
begin
  with DM_Clasif_E do begin
    if not Tbdescrip.Active then Exit; // evita error si la tabla está cerrada

    Tbdescrip.Edit;

    // Copia del servicio seleccionado
    TbdescripServicio.AsString := Tb_servicioServicio.AsString;

    // Genera el código EAN
    code := DM_Clasif_E.tbCodigoEANCodigo.AsString + generatecode(5);
    fullCode := code + IntToStr(CalculoDigitoControlEAN(code));

    TbdescripCodigoProducto.AsString := fullCode;

    Tbdescrip.Post;
  end;
end;

procedure TClasificador.btInsertServicesClick(Sender: TObject);
begin

  DM_Clasif_E.Insert.sql.Text :=
    'INSERT INTO tb_Servicio (Servicio) VALUES (:Servicio)';
  DM_Clasif_E.Insert.ParamByName('Servicio').AsString := edInServices.Text;
  DM_Clasif_E.Insert.ExecSQL;

  DM_Clasif_E.Tb_servicio.Refresh;

end;

procedure TClasificador.cxButton11Click(Sender: TObject);
begin
  DM_Clasif_E.Insert.sql.Text :=
    'INSERT INTO tbServiceDescrip (Descripcion, Habilitado, CodigoProducto) ' +
    'VALUES (:Descripcion, :Habilitado, :CodigoProducto)';

  DM_Clasif_E.Insert.ParamByName('Descripcion').AsString := edIDescrip.Text;
  DM_Clasif_E.Insert.ParamByName('Habilitado').AsBoolean := True;
  // Ajustable si es necesario
  DM_Clasif_E.Insert.ParamByName('CodigoProducto').AsString :=
    'Valor_CodigoProducto';
  DM_Clasif_E.Insert.ExecSQL;
...
begin
  ...
  DM_Clasif_E.tbInventarioINSUMO.Text := DM_Clasif_E.TbdescripDescripcion.Text;
...
begin
  for i := 1 to DM_Clasif_E.Tb_servicio.RecordCount - 1 do begin
    ...
    DM_Clasif_E.Tb_servicioScript.Params[1].AsString :=
      DM_Clasif_E.Tb_serviciocolor.text;
...
var i: Integer; sql: string; vJSON: TJSONObject;
begin
  ...
  //
  // edTipoMP.Clear
  DM_Clasif_E.tbGastosFijos.Post
end;
...
procedure TClasificador.cxButton8Click(Sender: TObject);
begin
  Ftasa.refresh_tasas;
  DM_Clasif_E.tb_Precios.Edit;
  DM_Clasif_E.tb_PreciosPrecio.Value :=
    StrToCurr(DM_Clasif_E.tb_PreciosDetalles.Value) *
    dmrate.tbCambioActualUSD.Value;
  DM_Clasif_E.tb_Precios.Post;
  DM_Clasif_E.tb_Precios.Refresh;
end;

procedure TClasificador.cxButton9Click(Sender: TObject);
var code12: string;
begin
  code12 := '';
  DM_Clasif_E.Tbdescrip.Append;
  DM_Clasif_E.Tbdescrip.Edit;
  DM_Clasif_E.TbdescripHabilitado.Value := True;
  DM_Clasif_E.TbdescripServicio.Value := DM_Clasif_E.Tb_servicioServicio.Value;

  // DM_Clasif_E.Tbdescripcolor.Value := DM_Clasif_E.Tb_serviciocolor.Value;

  code12 := DM_Clasif_E.tbCodigoEANCodigo.Text + generatecode(5);

  DM_Clasif_E.TbdescripCodigoProducto.Text := EAN13(code12.ToInt64());

  DM_Clasif_E.Tbdescrip.Post;
  DM_Clasif_E.Tbdescrip.Refresh;
  DM_Clasif_E.Tbdescrip.Refresh;
end;
...
var numeracion: string; Par: Integer; Impar: Integer; sumaTotal: Integer;

  digicontrol, digitcontrol2: Integer; sumatotalS: string; Multiplo: Integer;
  code: string;

begin

  // Para Hallar el digito de control de codebar
  if DM_Clasif_E.TbdescripCodigoProducto.Value = '' then begin
    Randomize;
    ...
    if digicontrol = 10 then digicontrol := 0;
...
var Fecha: Integer; df: string; wAnyo, wMes, wDia: Word; dateFind: Tdate;
  week: Integer; weekW: string;
begin
  df := '30/06/2022';
  DecodeDate(StrToDate(df), wAnyo, wMes, wDia);
  // ShowMessage(FormatDateTime('dd/MM/yyyy', Fecha+wMes+wAnyo));
  for Fecha := 1 to wDia do begin
    ...
    if DayOfTheWeek(dateFind) = 7 then begin
      week := week + 1;
...
function appdirectory(directoy: string): string;

begin
  Result := ExtractFilePath(Application.ExeName) + directoy
end;

procedure TClasificador.Button6Click(Sender: TObject);
begin
  loadIcon.Execute();
  DM_Clasif_E.Tb_servicio.Edit;
  DM_Clasif_E.Tb_servicioicon.LoadFromFile(loadIcon.FileName);
  DM_Clasif_E.Tb_servicio.Post;
  //
  // if not DirectoryExists(appdirectory('img\'+UnitName)) then
  // begin
  // CreateDir(appdirectory('img\'+UnitName))
  // end;
  // ShowMessage(appdirectory('img\'+UnitName))

  // dbimg.Picture.SaveToFile(ExtractFilePath(Application.ExeName)+ 'img\'+ unitname+'\'+DM_Clasif_E.Tb_servicioid.Value.ToString+'.png' )
...
begin
  if (sumaTotal < 10) then begin
    Multiplo := 10;
  end;
  if (sumaTotal >= 10) and (sumaTotal < 20) then begin
    Multiplo := 20;
  end;
  if (sumaTotal >= 20) and (sumaTotal < 30) then begin
    Multiplo := 30;
  end;
  if (sumaTotal >= 20) and (sumaTotal < 30) then begin
    Multiplo := 30;
  end;
  if (sumaTotal >= 30) and (sumaTotal < 40) then begin
    Multiplo := 40;
  end;
  if (sumaTotal >= 40) and (sumaTotal < 50) then begin
    Multiplo := 50;
  end;
  if (sumaTotal >= 50) and (sumaTotal < 60) then begin
    Multiplo := 60;
  end;
  if (sumaTotal >= 60) and (sumaTotal < 70) then begin
    Multiplo := 70;
  end;
  if (sumaTotal >= 70) and (sumaTotal < 80) then begin
    Multiplo := 80;
  end;
  if (sumaTotal >= 80) and (sumaTotal < 90) then begin
    Multiplo := 90;
  end;
  if (sumaTotal >= 90) and (sumaTotal < 100) then begin
    Multiplo := 100;
  end;
  if (sumaTotal >= 100) and (sumaTotal < 110) then begin
    Multiplo := 110;
  end;
  if (sumaTotal >= 110) and (sumaTotal < 120) then begin
    Multiplo := 120;
  end;
  if (sumaTotal >= 120) and (sumaTotal < 130) then begin
    Multiplo := 130;
  end;
  if (sumaTotal >= 130) and (sumaTotal < 140) then begin
    Multiplo := 140;
  end;
  if (sumaTotal >= 140) and (sumaTotal < 150) then begin
    Multiplo := 150;
  end;
  if (sumaTotal >= 150) and (sumaTotal < 160) then begin
    Multiplo := 160;
  end;
  if (sumaTotal >= 160) and (sumaTotal < 170) then begin
    Multiplo := 170;
  end;
  if (sumaTotal >= 170) and (sumaTotal < 180) then begin
    Multiplo := 180;
  end;
  if (sumaTotal >= 180) and (sumaTotal < 190) then begin
    Multiplo := 190;
  end;
  if (sumaTotal >= 190) and (sumaTotal < 200) then begin
    Multiplo := 200;
  end;
  if (sumaTotal >= 200) and (sumaTotal < 210) then begin
    Multiplo := 210;
  end;
  if (sumaTotal >= 210) and (sumaTotal < 220) then begin
    Multiplo := 220;
  end;
  if (sumaTotal >= 220) and (sumaTotal < 230) then begin
    Multiplo := 230;
  end;
  if (sumaTotal >= 230) and (sumaTotal < 240) then begin
    Multiplo := 240;
  end;
  if (sumaTotal >= 240) and (sumaTotal < 250) then begin
    Multiplo := 250;
  end;
  if (sumaTotal >= 250) and (sumaTotal < 260) then begin
    Multiplo := 260;
  end;
  if (sumaTotal >= 260) and (sumaTotal < 270) then begin
    Multiplo := 270;
  end;
  if (sumaTotal >= 270) and (sumaTotal < 280) then begin
    Multiplo := 280;
  end;
  if (sumaTotal >= 280) and (sumaTotal < 290) then begin
    Multiplo := 290;
  end;
  if (sumaTotal >= 290) and (sumaTotal < 300) then begin
    Multiplo := 300;
  end;
  if (sumaTotal >= 300) and (sumaTotal < 310) then begin
    Multiplo := 310;
  end;
  if (sumaTotal >= 310) and (sumaTotal < 320) then begin
    Multiplo := 320;
  end;
  if (sumaTotal >= 320) and (sumaTotal < 330) then begin
    Multiplo := 330;
  end;
  if (sumaTotal >= 330) and (sumaTotal < 340) then begin
    Multiplo := 340;
  end;
  if (sumaTotal >= 340) and (sumaTotal < 350) then begin
    Multiplo := 350;
  end;
  if (sumaTotal >= 350) and (sumaTotal < 360) then begin
    Multiplo := 360;
  end;
  if (sumaTotal >= 360) and (sumaTotal < 370) then begin
    Multiplo := 370;
  end;
  if (sumaTotal >= 370) and (sumaTotal < 380) then begin
    Multiplo := 380;
  end;
  if (sumaTotal >= 380) and (sumaTotal < 390) then begin
    Multiplo := 390;
  end;
  if (sumaTotal >= 390) and (sumaTotal < 400) then begin
    Multiplo := 400;
  end;
  if (sumaTotal >= 400) and (sumaTotal < 410) then begin
    Multiplo := 410;
  end;
  if (sumaTotal >= 410) and (sumaTotal < 420) then begin
    Multiplo := 420;
  end;
  if (sumaTotal >= 420) and (sumaTotal < 430) then begin
    Multiplo := 430;
  end;
  if (sumaTotal >= 430) and (sumaTotal < 440) then begin
    Multiplo := 440;
  end;
  if (sumaTotal >= 440) and (sumaTotal < 450) then begin
    Multiplo := 450;
  end;
  if (sumaTotal >= 450) and (sumaTotal < 460) then begin
    Multiplo := 460;
  end;
  if (sumaTotal >= 460) and (sumaTotal < 470) then begin
    Multiplo := 470;
  end;
  if (sumaTotal >= 470) and (sumaTotal < 480) then begin
    Multiplo := 480;
  end;
  if (sumaTotal >= 480) and (sumaTotal < 490) then begin
    Multiplo := 490;
  end;
  if (sumaTotal >= 490) and (sumaTotal < 500) then begin
    Multiplo := 500;
  end;
  if (sumaTotal >= 500) and (sumaTotal < 510) then begin
    Multiplo := 510;
  end;
  if (sumaTotal >= 510) and (sumaTotal < 520) then begin
    Multiplo := 520;
  end;
  if (sumaTotal >= 520) and (sumaTotal < 530) then begin
    Multiplo := 530;
  end;
  if (sumaTotal >= 530) and (sumaTotal < 540) then begin
    Multiplo := 540;
  end;
  if (sumaTotal >= 540) and (sumaTotal < 550) then begin
    Multiplo := 550;
  end;
end;
